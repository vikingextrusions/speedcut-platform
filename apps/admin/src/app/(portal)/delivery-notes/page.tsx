...
export default async function AdminDeliveryNotesPage() {
  ...
  const { data: deliveryNotes } = await supabase
    ...
    .select(`
      ...
      orders(order_number, organizations!orders_customer_org_id_fkey(name))
    `)
  ...
  return (
    <div style={{ animation: 'fade-in 0.3s ease-out' }}>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginTop: '0.25rem' }}>Track all deliveries across the platform</p>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        {deliveryNotes && deliveryNotes.length > 0 ? (
              ...
              <tbody>
                {deliveryNotes.map((dn: any) => (
                  <tr key={dn.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={tdStyle}>
                      <Link href={`/delivery-notes/${dn.id}`} style={{ color: 'var(--accent)', fontWeight: 600, textDecoration: 'none' }}>{dn.dn_number}</Link>
                    </td>
          ...
          <div style={{ padding: '4rem 2rem', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Delivery notes will appear here</p>
          </div>
        )}
      </div>
    </div>
