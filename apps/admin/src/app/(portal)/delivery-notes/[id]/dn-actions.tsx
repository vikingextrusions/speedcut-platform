...
const statusActions: Record<string, { label: string; next: string; color: string }[]> = {
  pending: [{ label: 'Mark Dispatched', next: 'dispatched', color: '#3b82f6' }],
  dispatched: [{ label: 'Mark Delivered', next: 'delivered', color: '#10b981' }],
  delivered: [{ label: 'Mark Signed', next: 'signed', color: '#10b981' }],
}

export function DNStatusActions({ dnId, currentStatus }: { dnId: string; currentStatus: string }) {
    ...
    return (
      <div className="card">
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>
          {currentStatus === 'signed' ? '✅ This delivery has been signed.' : 'No actions available.'}
        </p>
      </div>
    )
  }
