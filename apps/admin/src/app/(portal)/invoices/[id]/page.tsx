...
export default async function AdminInvoiceDetailPage({
  ...
  const { data: invoice } = await supabase
    ...
    .select(`
      ...
      vat_rate, shipping_cost, additional_costs, notes, created_at,
      orders(id, order_number, organizations!orders_customer_org_id_fkey(id, name)),
      invoice_lines(id, description, material, quantity, unit_price, sort_order),
      credit_notes(id, credit_note_number, status, total_amount)
    `)
    .eq('id', id)
    .single()
  ...
  const customerName = (invoice.orders as any)?.organizations?.name || '—'

  return (
    <div style={{ animation: 'fade-in 0.3s ease-out' }}>
      <PageHeader
        title={invoice.invoice_number}
        subtitle={`${customerName} · Issued ${new Date(invoice.invoice_date).toLocaleDateString()}`}
        backHref="/invoices"
          ...
          <>
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.875rem' }}>
                ...
                {invoice.shipping_cost > 0 && <Row label="Shipping" value={`£${Number(invoice.shipping_cost).toFixed(2)}`} />}
                <Row label={`VAT (${invoice.vat_rate || 20}%)`} value={`£${invoice.vat_amount.toFixed(2)}`} />
              ...
              <div className="card">
                ...
                {creditNotes.map((cn: any) => (
                  <Link key={cn.id} href={`/credit-notes/${cn.id}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0', textDecoration: 'none', color: 'inherit', fontSize: '0.85rem' }}>
                    <span style={{ color: 'var(--accent)', fontWeight: 600 }}>{cn.credit_note_number}</span>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)' }}>
                    {['#', 'Description', 'Material', 'Qty', 'Unit Price', 'Total'].map(h => (
